#!/bin/bash
#
# @file run_fitted_current_experiment.sh
# @brief Launches the fitted-Ekman / fitted-Gauss-Markov scenarios in
#        sequence, mirroring run_bluerov_current_experiment.sh exactly (same
#        launch/poll/verify loop, same timeouts) -- see that script's header
#        for how each run is stopped and verified. Kept as a separate script
#        rather than appended to the SCENARIOS array there so the base
#        experiment's array stays untouched and this can be re-run alone.
#
# Prerequisite: fit_ekman_profile.py, fit_gauss_markov_profile.py,
# generate_fitted_ekman_assets.py, generate_fitted_scenarios.py and
# generate_seed_variants.py (5 seeds, on each *_fitted_gauss_<date>.json)
# must already have been run -- see this directory's README.
#
# Must be run from the repository root, same as scenario_launch.sh itself:
#   src/simulation_run/scripts/bluerov_current_experiment/run_fitted_current_experiment.sh
...
SCENARIOS=()
for mission in station_keeping transect; do
    for date in "${DATES[@]}"; do
        SCENARIOS+=("${mission}_fitted_ekman_${date}.json")
    done
done
for mission in station_keeping transect; do
    for date in "${DATES[@]}"; do
        SCENARIOS+=("${mission}_fitted_gauss_${date}.json")
        for seed in 2 3 4 5; do
            SCENARIOS+=("${mission}_fitted_gauss_${date}_seed${seed}.json")
        done
    done
done
# 10 fitted-ekman + 50 fitted-gauss (10 base + 40 seed variants) = 60.
...
info "Batch done. Aggregating the fitted-Gauss-Markov ensembles (depth-uniform, seed variants)..."
for mission in station_keeping transect; do
    for date in "${DATES[@]}"; do
        python3 "$SELF_DIR/aggregate_seeds.py" "${mission}_fitted_gauss_${date}" \
            || warn "${mission}_fitted_gauss_${date} aggregation skipped/failed -- see above."
    done
done
