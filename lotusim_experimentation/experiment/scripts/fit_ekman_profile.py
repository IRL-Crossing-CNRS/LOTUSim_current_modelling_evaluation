...
This is the script that produced `scenarios/fitted_params_deep/`. It is
checked against four published dates spanning both regimes (2023-11-04,
2024-08-07, 2024-11-30, 2026-01-24): current velocity, orientation,
top-layer thickness, U10 and RMSE all reproduce to the precision the JSON
files were written at. `bottom_layer_thickness_m` is the one parameter that
does not reproduce exactly, on dates where the top layer already covers every
...
parameter has no effect on the fit and different optimiser runs settle on
different, equally arbitrary values for it. That is the same
collapsed/resolved distinction this study's results report, visible in the
fitting code itself rather than only in its output.

Two details of the recipe are worth stating explicitly, since each moves
every fitted value:

1. Residuals are UNWEIGHTED, not depth-weighted. Weighting them for
   Copernicus's geometric depth spacing (depth_weights()) shifts every
   fitted parameter measurably -- 2023-11-04's current_velocity_ms moves
   from 0.223 to 0.142 -- so the choice has to be made explicitly. The
   tracked results are unweighted.
...
`getBottomLayerCurrent`), so the fit and the simulation evaluate the same
model. The top-layer wind-driven speed
`V0 = sqrt(2)*pi*wind_stress/(top_layer_m*F_TIMES_RHO)` is inversely
proportional to `top_layer_m`, and the wind stress enters as a rotation of
`(windTauX, windTauY)`, which leaves its magnitude unchanged at any wind
angle. The same form is in `lotusim_sdk`'s `EkmanCurrentModel`, which the
analysis scripts evaluate; keep the three in step.
...
U10 is FITTED, not fixed: Gauss-Markov's mean/std are fit freely, so pinning
U10 while Ekman's other four parameters are free would be an asymmetry
between the two models' fitting procedures. It is therefore a fitted
coefficient of the current profile, not a wind measurement -- see
extract_copernicus_wind.py, which reports the measured wind separately.
...
# Coriolis parameter times water density: classical Ekman theory gives
# V0 = tau / (rho * sqrt(Az*f)) with D = pi*sqrt(2*Az/f), i.e.
# V0 = sqrt(2)*pi*tau / (rho * f * D) -- rho, and the result is a velocity.
