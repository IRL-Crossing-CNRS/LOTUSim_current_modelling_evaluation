...
Companion to fit_ekman_profile.py, and the script that produced the
`gauss_*.json` half of `scenarios/fitted_params_deep/`. Checked against four
published dates (2023-11-04, 2024-06-03, 2024-08-07, 2026-01-24) to the
precision the JSON files were written at. Two details of the recipe, each of
which moves the fitted values:
  ...
  * `mean_x`, `mean_y`: the PLAIN (unweighted) arithmetic mean of the
    ...
    depth-weighted for Copernicus's geometric spacing, which would move the
    fitted mean measurably. The tracked results are unweighted, matching the
    Ekman fit.
