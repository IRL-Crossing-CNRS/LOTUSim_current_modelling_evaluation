...
CURRENT profile. Nothing constrains it to equal the true wind, and the two
are not interchangeable -- on 2023-11-04, for instance, the measured wind is
17.45 m/s (Beaufort 8, consistent with Storm Ciaran, which made landfall in
Brittany 2023-11-01/02) while the fitted coefficient is far smaller.

The wind reported here is the measured one, read from `wind.nc` -- the
Copernicus wind product downloaded alongside the current export, independent
of any fit. That independence is the point: it lets each date's conditions be
stated, and the dates be screened, without reference to any model's fitted
parameters. It is the source of the Beaufort column in the reported tables.
