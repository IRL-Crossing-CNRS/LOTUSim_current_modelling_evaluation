...
#
# Every run is verified by the output file it is supposed to produce, and
# anything still missing at the end is listed, so a batch reports what it
# actually simulated rather than what it attempted.
#
# Usage:
...
SCEN_WS="${SCENARIO_WS:-$HOME/Documents/workspace/lotusim}/LOTUSim-generic-scenario"
LOTUSIM_WS="${LOTUSIM_WS:-$HOME/lotusim_ws}"
...
exits before simulating when the build is absent, so a batch started in this
state would produce no runs.
